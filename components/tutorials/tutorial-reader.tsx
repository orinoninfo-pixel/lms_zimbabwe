...
import { Bookmark, BookmarkCheck, Check, CheckCircle2, ChevronLeft, ChevronRight, Lightbulb, Menu, Search, TriangleAlert } from "lucide-react"
...
function TutorialNav({ tutorialSlug, sections, currentLessonId, completed, onNavigate }: { tutorialSlug: string; sections: Section[]; currentLessonId: string; completed: Set<string>; onNavigate?: () => void }) {
  ...
  return <div className="space-y-5"><div className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search this tutorial" className="pl-9" aria-label="Search tutorial lessons" /></div>{sections.map((section) => {
    ...
    return <section key={section.id}><h3 className="mb-2 text-xs font-semibold uppercase tracking-wider text-muted-foreground">{section.title}</h3><div className="space-y-1">{lessons.map((lesson) => <Link key={lesson.id} href={`/learn/${tutorialSlug}/${lesson.slug}`} onClick={onNavigate} className={cn("flex items-center gap-2 rounded-md px-3 py-2 text-sm transition-colors", currentLessonId === lesson.id ? "bg-primary text-primary-foreground" : "text-foreground/75 hover:bg-muted hover:text-foreground")}><span className="min-w-0 flex-1 truncate">{lesson.title}</span>{completed.has(lesson.id) ? <CheckCircle2 className="h-4 w-4 shrink-0" /> : null}</Link>)}</div></section>
  })}</div>
}
...
export function TutorialReader({ tutorial, currentLesson, completedLessonIds, bookmarked, authenticated }: { tutorial: { id: string; slug: string; title: string; sections: Section[] }; currentLesson: Lesson; completedLessonIds: string[]; bookmarked: boolean; authenticated: boolean }) {
  ...
  return <div className="min-h-screen bg-background pt-16">
    <aside className="fixed bottom-0 left-0 top-16 hidden w-72 overflow-y-auto border-r border-border bg-card p-5 lg:block"><Link href={`/learn/${tutorial.slug}`} className="mb-5 block text-lg font-semibold">{tutorial.title}</Link><TutorialNav tutorialSlug={tutorial.slug} sections={tutorial.sections} currentLessonId={currentLesson.id} completed={completed} /></aside>
    <main className="lg:ml-72 xl:mr-64"><div className="mx-auto max-w-4xl px-4 py-8 sm:px-7 lg:py-12">
      <div className="mb-7 flex items-center justify-between gap-3"><div className="flex items-center gap-2 text-sm text-muted-foreground"><Link href="/learn" className="hover:text-foreground">Free Learning</Link><span>/</span><span>{tutorial.title}</span></div><Sheet><SheetTrigger asChild><Button variant="outline" size="sm" className="lg:hidden"><Menu className="mr-2 h-4 w-4" />Lessons</Button></SheetTrigger><SheetContent side="left" className="w-[88vw] max-w-sm overflow-y-auto"><SheetHeader><SheetTitle>{tutorial.title}</SheetTitle></SheetHeader><div className="mt-6"><TutorialNav tutorialSlug={tutorial.slug} sections={tutorial.sections} currentLessonId={currentLesson.id} completed={completed} /></div></SheetContent></Sheet></div>
